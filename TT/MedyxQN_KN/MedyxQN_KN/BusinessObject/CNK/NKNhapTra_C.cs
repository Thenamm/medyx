...
{	
	...
	[Serializable()]
	public partial class NKNhapTra_C : BusinessBase<NKNhapTra_C>
	{
        ...
        protected override void AddBusinessRules()
        {
           ...
           // ValidationRules.AddRule(Csla.Validation.CommonRules.StringMaxLength, new Csla.Validation.CommonRules.MaxLengthRuleArgs("MaVT", 5));
            //ValidationRules.AddRule<NKNhapTra_C>(StringRequired<NKNhapTra_C>, "MaVT");
            ...
            ValidationRules.AddRule<NKNhapTra_C>(StringRequiredTenTM<NKNhapTra_C>, "TenTM");


        }
        //private static bool StringRequired<T>(T target, Csla.Validation.RuleArgs e) where T : NKNhapTra_C
        //{
        ...
        private static bool StringRequiredTenTM<T>(T target, Csla.Validation.RuleArgs e) where T : NKNhapTra_C
        {
        ...
        public static NKNhapTra_C NewNKNhapTra_C()
        {
            return new NKNhapTra_C();
            
        }

        public static NKNhapTra_C NewNKNhapTra_C(string idDMCNK)
        {
            return new NKNhapTra_C(DMCNK.GetDMCNK(idDMCNK));
        }

        public static NKNhapTra_C NewNKNhapTra_C(DMCNK _DMCNK)
        {
            return new NKNhapTra_C(_DMCNK);
            //return DataPortal.Create<DMDichVu_PhieuIn_C>(DMDichVu.GetDMDichVu(idDMDichvu));
        }
        public static NKNhapTra_C GetNKNhapTra_C(String maVT, Byte sTT)
        {
            return DataPortal.Fetch<NKNhapTra_C>(new Criteria(maVT, sTT));
        }

        public static NKNhapTra_C GetNKNhapTra_C(SafeDataReader dr)
        {
            return new NKNhapTra_C(dr);
        }
        ...
        protected internal NKNhapTra_C()
        { 
            //
        }
        protected NKNhapTra_C(DMCNK pro)
        {
            MarkAsChild();
            _maVT = pro.MaVT;
            _tenTM = pro.TenTM;
        }
        protected NKNhapTra_C(string _idMaPNT, DMCNK pro)
        {
            MarkAsChild();
            _MaPNT = _idMaPNT;            
            _maVT = pro.MaVT;
        ...
        protected NKNhapTra_C(SafeDataReader dr)
        {
            MarkAsChild();
            ...
            //_tonCk = HTC.Business.CategoryList.DMCNK_TonKho.GetDMCNKTonKho(_maVT, _ngayn).TonCK;
        ...
        {
            using (SafeDataReader dr = new SafeDataReader(DataProvider.Instance().GetNKNhapTra_C(crit.MaPNT, crit.STT))) 
            {
                if (dr.Read())
                {
                    Fetch(dr);
        ...
        internal virtual void Insert(NKNhapTra bill)
        {
            
              //InsertNKNhapTra_C(String _MaPNT, String _maVT, Decimal _soLuong, SmartDate _hanSD,  Decimal _donGia,String _soLo,  Boolean Huy, String _ghiChu,String _maMay, String _nguoiSD);
            bill.MaPNT = DataProvider.Instance().InsertNKNhapTra_C(bill.MaPNT, _maVT, _soLuong, _hanSD, _donGia, _soLo,_huy, _ghiChu,  _maMay, _nguoiSD, _soLuongYC);
            MarkOld();
        }

        internal virtual void Update(NKNhapTra bill)
        {
            if(IsDirty)
            {      
            
              
            DataProvider.Instance().UpdateNKNhapTra_C(bill.MaPNT, _sTT, _maVT, _soLuong, _hanSD, _soLo, _donGia,  _soLuongYC, _giaBan,  _huy,  _ghiChu,  _nguoiSD,  _maMay);
            }
        }
        internal void DeleteSelf(NKNhapTra packet)
        {
            if (!this.IsDirty)
                return;
            if (this.IsNew)
                return;
            DataProvider.Instance().DeleteNKNhapTra_C(_MaPNT,_sTT,_nguoiSD,_maMay);
            MarkNew();
