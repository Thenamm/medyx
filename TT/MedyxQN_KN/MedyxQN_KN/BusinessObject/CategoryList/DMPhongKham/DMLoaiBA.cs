...
{	
	...
	[Serializable()]
	public partial class DMLoaiBA : BusinessBase<DMLoaiBA>
	{
		...
		private String _tenLoaiBA = String.Empty;		
		...
		public String TenLoaiBA
		{
			get 
			{
				CanReadProperty("TenLoaiBA", true);
				return _tenLoaiBA;
			}
			set 
			{
				CanWriteProperty("TenLoaiBA", true);
				if (_tenLoaiBA != value) 
				{
					_tenLoaiBA = value;
					PropertyHasChanged("TenLoaiBA");
				}
			}
		}
		...
		/// </summary> 
        /// 
        public static DMLoaiBA NewDMLoaiBA()
        {
            //return new DMLoaiBA();
            return DataPortal.Create<DMLoaiBA>();
        }
		public static DMLoaiBA GetDMLoaiBA(String maLoaiBA)
		{
			return DataPortal.Fetch<DMLoaiBA>(new Criteria(maLoaiBA));
		}
		
		/// <summary>
		/// Marks the <see cref="DMLoaiBA" /> Object for deletion. The Object will be deleted as part of the next save operation.
		/// </summary>
        public static void DeleteDMLoaiBA(String maLoaiBA, string mamay, string nguoihuy)
		{
            DataPortal.Delete(new OtherCriteria(mamay,nguoihuy ,maLoaiBA));
		}
		...
		protected internal DMLoaiBA()
		{	
			// Prevent direct creation
		}
        internal DMLoaiBA(int ordernumber, string MaLoaiBA, string TenLoaiBA,string idmay , bool huy,SmartDate ngaySD, string NguoiSD,string ghichu ,String hoten)
        {
            _OrderNumber = ordernumber;
            _maLoaiBA = MaLoaiBA ;
            _tenLoaiBA  = TenLoaiBA;       
            _maMay = idmay;
            _huy = huy;
            _ngaySD = ngaySD;
            _nguoiSD = NguoiSD;
		...
		protected class Criteria
		{
			...
			public Criteria(String maLoaiBA)
			{
				_maLoaiBA = maLoaiBA;
			}
			...
			public override bool Equals(object obj)
			{
				if (obj is Criteria)
				{
					...
					if (!_maLoaiBA.Equals(c._maLoaiBA)) 
						return false;
					return true;
				}
				return false;
			...
			{
				return string.Concat("Criteria", _maLoaiBA.ToString()).GetHashCode();
			}
		}
        ...
        protected class OtherCriteria
        {
            ...
            public OtherCriteria(String Mamay, String Nguoisd, String maLoaiBA)
            {
                _maLoaiBA = maLoaiBA;
                _maMay = Mamay;
                _NguoiSD = Nguoisd;
		...
		protected void DataPortal_Fetch(Criteria crit)
		{
			...
			// }
			using (SafeDataReader dr = new SafeDataReader(DataProvider.Instance().GetDMLoaiBA(crit.MaLoaiBA)))
			{
				if (dr.Read()){
					Fetch(dr);
					MarkOld();
		...
		private void Fetch(SafeDataReader dr)
		{
			...
			if (dr.GetString("TenLoaiBA") != null) _tenLoaiBA = dr.GetString("TenLoaiBA");
		...
		protected override void DataPortal_Insert()
		{
			...
			_maLoaiBA = DataProvider.Instance().InsertDMLoaiBA(_tenLoaiBA, _ghiChu, _maMay, _nguoiSD);
		...
		protected override void DataPortal_Update()
		{
			...
			if (IsDirty){
				...
				DataProvider.Instance().UpdateDMLoaiBA(_maLoaiBA, _tenLoaiBA, _ghiChu, _maMay, _nguoiSD , _huy );
		...
		{
            DataPortal_Delete(new OtherCriteria(_maMay, _nguoiSD , _maLoaiBA));
		}
		...
		{
			// TODO: Kiem tra lai cac tham so truyen vao cho ham UpdateDMLoaiBA
            DataProvider.Instance().DeleteDMLoaiBA(crit.MaMay, crit.NguoiSD, crit.MaLoaiBA);
