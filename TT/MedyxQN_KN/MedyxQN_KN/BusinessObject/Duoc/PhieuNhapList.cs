...
{
    [Serializable()]
    public class PhieuNhapList : BusinessListBase<PhieuNhapList, PhieuNhap >
    {
         #region Factory Methods
        public static PhieuNhapList GetAllPhieuNhap(Boolean _qadmin, String _maKho, String _mNCC, DateTime _tuNgay, DateTime _denNgay, String _dKien)
        {
            return DataPortal.Fetch<PhieuNhapList>(new Criteria(_qadmin, _maKho, _mNCC, _tuNgay, _denNgay, _dKien));
        }
        public void NewTo(PhieuNhap _PhieuNhap)
        {
            _PhieuNhap.OrderNumber = this.Items[this.Count - 1].OrderNumber + 1;
            this.Add(_PhieuNhap);
        }
        public PhieuNhap NewTo()
        {
             PhieuNhap obj = PhieuNhap.NewPhieuNhap ();
              return obj;
        }
        public void UpdatedTo(PhieuNhap _PhieuNhap)
        {

            _PhieuNhap.OrderNumber = this.Items[this.IndexOf(_PhieuNhap)].OrderNumber;
            this.Items[this.IndexOf(_PhieuNhap)] = _PhieuNhap;
        }
        public void RemoveTo(PhieuNhap _PhieuNhap)
        {
            this.Items.Remove(_PhieuNhap);

        }
        private PhieuNhapList()
        ...
        private void DataPortal_Fetch(Criteria criteria)
        {
            ...
            using (SafeDataReader dr = new SafeDataReader(DataProvider.Instance().GetPhieuNhap_GetAll(criteria.qadmin, criteria.makho, criteria.maNCC, criteria.tungay, criteria.denngay, criteria.dk)))
            {
                ...
                int i = 1;
                //IsReadOnly = false;
                while (dr.Read())// phai trung voi trat tu cac truong ben PhieuNhap vao phuong thuc (internal PhieuNhap)
                {
                    try 
                    {
