...
namespace Dzual.CategoryList
{
	...
	public class Dzual_DMTBYT
	{
		...
		private Int32 _namSX = 0;
		private String _serialNo = "";
		private String _model = "";
		private String _maTinhTrang = "";
		private String _tanSuatSuDung = "";
		private Int32 _khauhao = 0;
        private Byte _maKHBaoTri = 0;
        private Boolean _baoTriDinhKy = false;
		...
		private Int32 _thoiGianTHKeHoach = 0;
		...
		public Dzual_DMTBYT(IDataReader dr, int Ordernumber)
		{
            ...
            if (dr["TenTBYT"] != null && dr["TenTBYT"].GetType().Name != "DBNull") _tenTBYT = Convert.ToString(dr["TenTBYT"]);
		...
		#region GetAll,Update
            ...
            /// <param name="Loai">Loại=1: Bảo trì; Loai=2: Sửa chữa</param>
            /// <returns></returns>
            public static DataSet GetDMTBYTBaoTri_SuaChua(String MaTB, Byte Loai)
            {
                return SqlHelper.ExecuteDataset(ConnectString, "dbo.spDMTBYT_SCHUABTRI_BYMATBYT", MaTB, Loai);
            }


        /// <summary>
        /// Kiểm tra xem thiết bị này đã được đưa vào sử dụng hay chưa, nếu rồi thì sẽ ko được sửa thông tin kho khoa
        /// ngược lại thì được sửa thông tin.
        /// </summary>
        /// <param name="MaTB"></param>
        /// <returns></returns>
        
            public static Boolean GetMaTBExistContain(String MaTB)
            {
                String strCommand = "DECLARE @MaTB1 nvarchar(13) SELECT @MaTB1 = ISNULL(XA.MaTB, ISNULL(SA.MaTB, BA.MaTB)) FROM DMTBYT A ";
                strCommand += " LEFT JOIN TBPhieuXuat XA ON A.MaTB = XA.MaTB LEFT JOIN TBPhieuSuaChua_C SA ON A.MaTB = SA.MaTB ";
                strCommand += " LEFT JOIN TBPhieuBaoTri_C BA ON A.MaTB = BA.MaTB WHERE ISNULL(A.Huy,0) = 0 AND A.MaTB = '" + MaTB + "' ";
                strCommand += " IF @@ROWCOUNT <= 1 AND ISNULL(@MaTB1,'') = '' SELECT 0 ELSE SELECT 1 ";
                 return System.Convert.ToBoolean(SqlHelper.ExecuteScalar(ConnectString, CommandType.Text, strCommand));
            }


            public static void Delete(String MaTB, String MaMay, String NguoiHuy)
            {
                SqlHelper.ExecuteNonQuery(ConnectString, "dbo.spDMTBYT_DELETED", MaMay, NguoiHuy, MaTB);
            }

            public static void Update(String MaTB, String TenTB, String MaDVT, String MaNhom, String MaChungLoai, String MaKho, String MaKhoa, String MaNoiSua, String MaQG, String MaBarCode
                , String MaNSX, int NamSX, String SerialNo, String Model, String MaTinhTrang, String TanSuatSuDung, int Khauhao, Byte MaKHBaoTri, Boolean BaoTriDinhKy,Nullable<DateTime> KHTuNgay,
                Nullable<DateTime> KHDenNgay, Decimal Gianhap, String NguonKP, DateTime NgayHopDong, String SoHopDong, String MaNCC, int ThoiGianBH, DateTime NgaySuDung, String AnhTB, String Ghichu,
                String NguoiSD, DateTime NgayMua
                )
            {
                   SqlHelper.ExecuteNonQuery(ConnectString, "dbo.spDMTBYT_UPDATE",
                    MaTB, TenTB, MaDVT, MaNhom, MaChungLoai, string.IsNullOrEmpty(MaKho) ? null : MaKho, string.IsNullOrEmpty(MaKhoa) ? null : MaKhoa, MaNoiSua, MaQG, MaBarCode,
                    MaNSX, NamSX, SerialNo, Model, MaTinhTrang, TanSuatSuDung, Khauhao == -1 ? new Nullable<int>() : Khauhao, MaKHBaoTri, BaoTriDinhKy,
                    KHTuNgay == DateTime.MinValue ? new Nullable<DateTime>() : KHTuNgay,
                    KHDenNgay == DateTime.MinValue ? new Nullable<DateTime>() : KHDenNgay, Gianhap == -1 ? new Nullable<decimal>() : Gianhap, NguonKP,
                    NgayHopDong == DateTime.MinValue ? new Nullable<DateTime>() : NgayHopDong, SoHopDong, MaNCC, ThoiGianBH == -1 ? new Nullable<int>() : ThoiGianBH, NgaySuDung == DateTime.MinValue ? new Nullable<DateTime>() : NgaySuDung, AnhTB, Ghichu,
                    NguoiSD, NgayMua == DateTime.MinValue ? new Nullable<DateTime>() : NgayMua);
            }

            public static String Insert(String TenTB, String MaDVT, String MaNhom, String MaChungLoai, String MaKho, String MaKhoa, String MaNoiSua, String MaQG, String MaBarCode
                , String MaNSX, int NamSX, String SerialNo, String Model, String MaTinhTrang, String TanSuatSuDung, int Khauhao, Byte MaKHBaoTri, Boolean BaoTriDinhKy, Nullable<DateTime> KHTuNgay,
                Nullable<DateTime> KHDenNgay, Decimal Gianhap, String NguonKP, DateTime NgayHopDong, String SoHopDong, String MaNCC, int ThoiGianBH, DateTime NgaySuDung, String AnhTB, String Ghichu,
                String MaMay, String NguoiLap, DateTime NgayMua
                )
            {
                return System.Convert.ToString(SqlHelper.ExecuteScalar(ConnectString, "dbo.spDMTBYT_CREATE",
                    TenTB, MaDVT, MaNhom, MaChungLoai, string.IsNullOrEmpty(MaKho)? null: MaKho, string.IsNullOrEmpty(MaKhoa)? null: MaKhoa, MaNoiSua, MaQG, MaBarCode,
                    MaNSX, NamSX, SerialNo, Model, MaTinhTrang, TanSuatSuDung, Khauhao == -1 ? new Nullable<int>() : Khauhao, MaKHBaoTri, BaoTriDinhKy,
                    KHTuNgay == DateTime.MinValue ? new Nullable<DateTime>() : KHTuNgay,
                    KHDenNgay == DateTime.MinValue ? new Nullable<DateTime>() : KHDenNgay, Gianhap == -1 ? new Nullable<decimal>() : Gianhap, NguonKP,
                    NgayHopDong == DateTime.MinValue ? new Nullable<DateTime>() : NgayHopDong, SoHopDong, MaNCC, ThoiGianBH == -1 ? new Nullable<int>() : ThoiGianBH, NgaySuDung == DateTime.MinValue ? new Nullable<DateTime>() : NgaySuDung, AnhTB, Ghichu,
                    MaMay, NguoiLap, NgayMua == DateTime.MinValue ? new Nullable<DateTime>() : NgayMua));
            }
