...
{	
	...
	[Serializable()]
	public partial class VPPNhapTra_C : BusinessBase<VPPNhapTra_C>
	{
        ...
        protected override void AddBusinessRules()
        {
           ...
           // ValidationRules.AddRule(Csla.Validation.CommonRules.StringMaxLength, new Csla.Validation.CommonRules.MaxLengthRuleArgs("MaVPP", 5));
            //ValidationRules.AddRule<VPPNhapTra_C>(StringRequired<VPPNhapTra_C>, "MaVPP");
            ...
            ValidationRules.AddRule<VPPNhapTra_C>(StringRequiredTenTM<VPPNhapTra_C>, "TenTM");


        }
        //private static bool StringRequired<T>(T target, Csla.Validation.RuleArgs e) where T : VPPNhapTra_C
        //{

        //    if (target._maVPP == null || target._maVPP.Trim().Length == 0)
        //    {
        ...
        private static bool StringRequiredTenTM<T>(T target, Csla.Validation.RuleArgs e) where T : VPPNhapTra_C
        {
        ...
        public static VPPNhapTra_C NewVPPNhapTra_C()
        {
            return new VPPNhapTra_C();
            
        }

        public static VPPNhapTra_C NewVPPNhapTra_C(string idDMVPPYT)
        {
            return new VPPNhapTra_C(DMVPP.GetDMVPP(idDMVPPYT));
        }

        public static VPPNhapTra_C NewVPPNhapTra_C(DMVPP _DMVPPYT)
        {
            return new VPPNhapTra_C(_DMVPPYT);
            //return DataPortal.Create<DMDichVu_PhieuIn_C>(DMDichVu.GetDMDichVu(idDMDichvu));
        }
        public static VPPNhapTra_C GetVPPNhapTra_C(String maVPP, Byte sTT)
        {
            return DataPortal.Fetch<VPPNhapTra_C>(new Criteria(maVPP, sTT));
        }

        public static VPPNhapTra_C GetVPPNhapTra_C(SafeDataReader dr)
        {
            return new VPPNhapTra_C(dr);
        }
        ...
        protected internal VPPNhapTra_C()
        { 
            //
        }
        protected VPPNhapTra_C(DMVPP pro)
        {
            MarkAsChild();
            _maVT = pro.MaVT;
            _tenTM = pro.TenTM;
        }
        protected VPPNhapTra_C(string _idMaPNT, DMVPP pro)
        {
            MarkAsChild();
            _MaPNT = _idMaPNT;            
            _maVT = pro.MaVT;
        ...
        protected VPPNhapTra_C(SafeDataReader dr)
        {
            MarkAsChild();
            ...
            //_tonCk = HTC.Business.CategoryList.DMVPPYT_TonKho.GetDMVPPYTTonKho(_maVPP, _ngayn).TonCK;
        ...
        {
            using (SafeDataReader dr = new SafeDataReader(DataProvider.Instance().GetVPPNhapTra_C(crit.MaPNT, crit.STT))) 
            {
                if (dr.Read())
                {
                    Fetch(dr);
        ...
        internal virtual void Insert(VPPNhapTra bill)
        {
            
              //InsertVPPNhapTra_C(String _MaPNT, String _maVPP, Decimal _soLuong, SmartDate _hanSD,  Decimal _donGia,String _soLo,  Boolean Huy, String _ghiChu,String _maMay, String _nguoiSD);
            _MaPNT = bill.MaPNT;
             DataProvider.Instance().InsertVPPNhapTra_C(bill.MaPNT, _maVT, _soLuong, _hanSD, _donGia, _soLo,_huy, _ghiChu,  _maMay, _nguoiSD , _soLuongYC);
        }

        internal virtual void Update(VPPNhapTra bill)
        {
            if(IsDirty)
            {      
            
              
            DataProvider.Instance().UpdateVPPNhapTra_C(bill.MaPNT, _sTT,_maVT , _soLuong, _hanSD, _soLo, _donGia,  _soLuongYC, _giaBan,  _huy,  _ghiChu, _nguoiSD ,  _maMay);
            }
        }
        internal void DeleteSelf(VPPNhapTra packet)
        {
            if (!this.IsDirty)
                return;
            if (this.IsNew)
                return;
            DataProvider.Instance().DeleteVPPNhapTra_C(_MaPNT,_sTT,_nguoiSD,_maMay);
            MarkNew();
