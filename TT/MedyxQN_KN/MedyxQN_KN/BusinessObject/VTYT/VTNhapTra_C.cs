...
{	
	...
	[Serializable()]
	public partial class VTNhapTra_C : BusinessBase<VTNhapTra_C>
	{
        ...
        protected override void AddBusinessRules()
        {
           ...
           // ValidationRules.AddRule(Csla.Validation.CommonRules.StringMaxLength, new Csla.Validation.CommonRules.MaxLengthRuleArgs("MaVT", 5));
            //ValidationRules.AddRule<VTNhapTra_C>(StringRequired<VTNhapTra_C>, "MaVT");
            ...
            ValidationRules.AddRule<VTNhapTra_C>(StringRequiredTenTM<VTNhapTra_C>, "TenTM");


        }
        //private static bool StringRequired<T>(T target, Csla.Validation.RuleArgs e) where T : VTNhapTra_C
        //{
        ...
        private static bool StringRequiredTenTM<T>(T target, Csla.Validation.RuleArgs e) where T : VTNhapTra_C
        {
        ...
        public static VTNhapTra_C NewVTNhapTra_C()
        {
            return new VTNhapTra_C();
            
        }

        public static VTNhapTra_C NewVTNhapTra_C(string idDMVTYT)
        {
            return new VTNhapTra_C(DMVTYT.GetDMVTYT(idDMVTYT));
        }

        public static VTNhapTra_C NewVTNhapTra_C(DMVTYT _DMVTYT)
        {
            return new VTNhapTra_C(_DMVTYT);
            //return DataPortal.Create<DMDichVu_PhieuIn_C>(DMDichVu.GetDMDichVu(idDMDichvu));
        }
        public static VTNhapTra_C GetVTNhapTra_C(String maVT, Byte sTT)
        {
            return DataPortal.Fetch<VTNhapTra_C>(new Criteria(maVT, sTT));
        }

        public static VTNhapTra_C GetVTNhapTra_C(SafeDataReader dr)
        {
            return new VTNhapTra_C(dr);
        }
        ...
        protected internal VTNhapTra_C()
        { 
            //
        }
        protected VTNhapTra_C(DMVTYT pro)
        {
            MarkAsChild();
            _maVT = pro.MaVT;
            _tenTM = pro.TenTM;
        }
        protected VTNhapTra_C(string _idMaPNT, DMVTYT pro)
        {
            MarkAsChild();
            _MaPNT = _idMaPNT;            
            _maVT = pro.MaVT;
        ...
        protected VTNhapTra_C(SafeDataReader dr)
        {
            MarkAsChild();
            ...
            //_tonCk = HTC.Business.CategoryList.DMVTYT_TonKho.GetDMVTYTTonKho(_maVT, _ngayn).TonCK;
        ...
        {
            using (SafeDataReader dr = new SafeDataReader(DataProvider.Instance().GetVTNhapTra_C(crit.MaPNT, crit.STT))) 
            {
                if (dr.Read())
                {
                    Fetch(dr);
        ...
        internal virtual void Insert(VTNhapTra bill)
        {
            
              //InsertVTNhapTra_C(String _MaPNT, String _maVT, Decimal _soLuong, SmartDate _hanSD,  Decimal _donGia,String _soLo,  Boolean Huy, String _ghiChu,String _maMay, String _nguoiSD);
            _MaPNT = bill.MaPNT;
             DataProvider.Instance().InsertVTNhapTra_C(bill.MaPNT, _maVT, _soLuong, _hanSD, _donGia, _soLo,_huy, _ghiChu,  _maMay, _nguoiSD , _soLuongYC);
             MarkOld();
        }

        internal virtual void Update(VTNhapTra bill)
        {
            if(IsDirty)
            {      
            
              
            DataProvider.Instance().UpdateVTNhapTra_C(bill.MaPNT, _sTT, _maVT, _soLuong, _hanSD, _soLo, _donGia,  _soLuongYC, _giaBan,  _huy,  _ghiChu, _nguoiSD ,  _maMay);
            }
        }
        internal void DeleteSelf(VTNhapTra packet)
        {
            if (!this.IsDirty)
                return;
            if (this.IsNew)
                return;
            DataProvider.Instance().DeleteVTNhapTra_C(_MaPNT,_sTT,_nguoiSD,_maMay);
            MarkNew();
