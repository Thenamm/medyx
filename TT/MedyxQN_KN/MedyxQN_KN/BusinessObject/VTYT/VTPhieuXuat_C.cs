...
{	
	...
	[Serializable()]
	public partial class VTPhieuXuat_C : BusinessBase<VTPhieuXuat_C>
	{
        ...
        public String tonkhoa
        {
            get
            {
                if (_ghiChu1.IndexOf("/") > 0)
                    return  _ghiChu1.Substring(_ghiChu1.IndexOf("/") + 1);
                else
                    return "0";
            }
        ...
        public String sodu
        {
            get
            {
                if (tonkho != "" && tonkhoa != "")
                {
                    Decimal a = 0;
                    Decimal b = 0;
                    Decimal.TryParse(tonkho, out  a);
                    Decimal.TryParse(tonkhoa, out b);
                    if (a != 0 || b != 0)
                        _ghiChu = "";
                    return (b - a).ToString("###,###.##");
                }
        ...
        public static VTPhieuXuat_C NewVTPhieuXuat_C()
        {
            return new VTPhieuXuat_C();
            
        }

        public static VTPhieuXuat_C NewVTPhieuXuat_C(string idDMVTYT)
        {
            return new VTPhieuXuat_C(DMVTYT.GetDMVTYT(idDMVTYT));
        }

        public static VTPhieuXuat_C NewVTPhieuXuat_C(DMVTYT _DMVTYT)
        {
            return new VTPhieuXuat_C(_DMVTYT);
            //return DataPortal.Create<DMDichVu_PhieuIn_C>(DMDichVu.GetDMDichVu(idDMDichvu));
        }
        
        public static VTPhieuXuat_C GetVTPhieuXuat_C(String maVTPhieuXuat, decimal sTT)
        {
            return DataPortal.Fetch<VTPhieuXuat_C>(new Criteria(maVTPhieuXuat, sTT));
        }

        public static VTPhieuXuat_C GetVTPhieuXuat_C(SafeDataReader dr)
        {
            return new VTPhieuXuat_C(dr);
        }

        public static void DeleteVTPhieuXuat_C(String _maPN, decimal _sTT, string _nguoiSD, string _mamay)
        {
        ...
        protected internal VTPhieuXuat_C()
        { 
            //
        }
        protected VTPhieuXuat_C(DMVTYT pro)
        {
            MarkAsChild();
            _maVT = pro.MaVT;
            _tenTM = pro.TenTM;
        }
        protected VTPhieuXuat_C(string _idMaPX, DMVTYT pro)
        {
            MarkAsChild();
            _maPX = _idMaPX;            
            _maVT = pro.MaVT;
        ...
        protected VTPhieuXuat_C(SafeDataReader dr)
        {
            MarkAsChild();
        ...
        internal virtual void Insert(VTPhieuXuat bill)
        {
            
              
            _maPX = bill.MaPX;
             DataProvider.Instance().InsertVTPhieuXuat_C(bill.MaPX, _maVT, _soLuong, _hanSD, _soLo, _donGia, _soLuongYC, _giaBan, _ghiChu, _nguoiSD , _maMay);
             MarkOld();
        }

        internal virtual void Update(VTPhieuXuat bill)
        {
            if(IsDirty)
            {      
            
              
            DataProvider.Instance().UpdateVTPhieuXuat_C(bill.MaPX, _sTT, _maVT, _soLuong, _hanSD, _soLo, _donGia,  _soLuongYC, _giaBan,  _huy,  _ghiChu, _nguoiSD ,  _maMay);
            }
        }
        internal void DeleteSelf(VTPhieuXuat packet)
        {
            if (!this.IsDirty)
                return;
            if (this.IsNew)
                return;
            DataProvider.Instance().DeleteVTPhieuXuat_C(_maPX,_sTT,_nguoiSD,_maMay);
            MarkNew();
            
        }
        internal virtual void Insert(VTPhieuXuatDT bill)
        {
        ...
        internal virtual void Update(VTPhieuXuatDT bill)
        {
            if (IsDirty)
            {
        ...
        internal void DeleteSelf(VTPhieuXuatDT packet)
        {
            if (!this.IsDirty)
                return;
            if (this.IsNew)
