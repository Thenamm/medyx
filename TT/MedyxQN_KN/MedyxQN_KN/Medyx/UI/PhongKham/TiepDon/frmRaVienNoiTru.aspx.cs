...
public partial class UI_NoiTru_KhamBenh_frmRaVienNoiTru : WebBase
{
    ...
    protected Boolean PrintdataTT(Boolean _load = true)
    {
        try
        {
            ...
            if (HTC.ShareVariables.pub_spC == "PS")
            {
                Myformulas["ChanDoanvv"].Text = "'" + _BenhAnPK.TenBenhKKB + "'";
                Myformulas["ChanDoan"].Text = "'" + _BenhAnPK.TenBenhRaVien + "'";
            ...
            Myformulas["LD"].Text = "'" + txtLoiDan.Text.Replace("'", "").Replace("\r\n", "|||||") + "'";
    ...
    protected Boolean Printdata(Boolean _load = true)
    {
        try
        {
            ...
            if (HTC.ShareVariables.pub_spC == "QN")
            {
                ...
                Myformulasrv["ttravien"].Text = "'" + txtKetQuaCLS.Text.Replace("\r\n", "|||||") + "'";
    ...
    private void PrintExport(ReportDocument rpt4, string v)
    {
        throw new NotImplementedException();
    }

    private void PrintExport(ReportDocument rptnvbh)
    {
        throw new NotImplementedException();
    }
    ...
    public bool CheckDT(string maba, int sttkhoa, string sttin, string dk)
    {
        bool kq = false;
        DataTable dr = new DataTable();
    ...
    public bool CheckDTDY(string maba, int sttkhoa, string sttin, string dk)
    {
        bool kq = false;
        DataTable dr = new DataTable();
            ...
            if (dtAC.Rows.Count > 0)
            {
                ...
                if (CheckDT(_MaBA, _STTKhoa, sttin, "") == false)
                {
                    ...
                    if (result == "{\"success\":\"Gửi đơn thuốc thành công\"}")
                    {
                        ...
                        string sql = "Update benhan_thuocSD set SoDTDT ='" + donthuoc.ma_don_thuoc + "' where isnull(huy,0)=0 and maba ='" + _MaBA + "' and sttkhoa=" + _STTKhoa + " and stt =" + sttin[i];
                        WriteLog("sql" + sql);
                        HTC.Business.DataProvider.Instance().ExcSQL(sql);
        ...
        try
        {
            ...
            if (dtAC.Rows.Count > 0)
            {
                ...
                if (CheckDTDY(_MaBA, _STTKhoa, sttin, "") == false)
                {
                    ...
                    if (result == "{\"success\":\"Gửi đơn thuốc thành công\"}")
                    {
                        string sql = "Update benhan_thuocSD_DY set SoDTDT ='" + donthuoc.ma_don_thuoc + "' where isnull(huy,0)=0 and maba ='" + _MaBA + "' and sttkhoa=" + _STTKhoa + " and stt in(" + sttin + ")";
                        WriteLog("sql" + sql);
                        HTC.Business.DataProvider.Instance().ExcSQL(sql);
                    }
                }
